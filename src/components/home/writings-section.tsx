...
export async function WritingsSection() {
	...
	// the viewport, instead of item 16 inheriting a ~0.9s absolute slot and
	// sitting invisible after scrolling in.
	const rowChunks: (typeof rows)[] = [];
	for (let i = 0; i < rows.length; i += 4) {
		rowChunks.push(rows.slice(i, i + 4));
	}
			...
			<div className="px-6 sm:px-8">
				...
				<div className="flex flex-col gap-y-8 sm:gap-y-10">
					{rowChunks.map((chunk) => (
						<ScrollReveal
							key={chunk[0].id}
							className="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2 sm:gap-y-10 lg:grid-cols-4"
							stagger={0.06}
						>
							{chunk.map((item) => (
