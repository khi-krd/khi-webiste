...
export async function LatestUpdates() {
	...
	return (
		...
		>
			<ScrollRevealBlock className="mb-8 px-6 sm:mb-10 sm:px-8 2xl:px-[calc((100vw-var(--canvas))/2+2rem)]">
				<header>
			...
			<div className="px-6 sm:px-8 2xl:px-[calc((100vw-var(--canvas))/2+2rem)]">
				{/* 2xl rows: clamp() only grows the 18rem floor on viewports tall
				    enough to spare it (32svh), so short 2xl laptops keep today's
				    heights and the section still fits one snap step. The 28rem
				    ceiling is for the 27" class — two rows at the old 22rem cap
				    filled barely half a 1440px-tall screen, and the section read
				    as a small card grid marooned in a lot of paper. */}
				<ScrollReveal className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-12 lg:grid-rows-[minmax(18rem,1fr)_minmax(18rem,1fr)] lg:gap-4 2xl:grid-rows-[minmax(clamp(18rem,32svh,28rem),1fr)_minmax(clamp(18rem,32svh,28rem),1fr)]">
					{hero ? (
